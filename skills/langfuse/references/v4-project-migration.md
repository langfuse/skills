...
- Map each deprecated endpoint per the guide's quick reference: trace, observation, and session reads move to `GET /v2/observations`; `GET /metrics` and `GET /metrics/daily` to `GET /v2/metrics`; score reads to `GET /v3/scores`; dataset runs to `GET /experiments` plus `GET /experiment-items`; event ingestion (`POST /ingestion` and the older synchronous routes) to `POST /otel/v1/traces` or an upgraded SDK. Score writes via `POST /scores` are not deprecated; leave them unchanged.
- Migrate parameters, pagination, filters, field groups, and response parsing together with the path. The replacements are not drop-in: pagination is cursor-based, responses omit unrequested field groups, and several endpoints require bounded time ranges. Apply the guide's per-endpoint parameter mapping instead of assuming v1 semantics.
- Rework consumers around the observations-first model where the guide requires it: traces and sessions come back as observation rows the client groups by `traceId` or `sessionId`, trace-level input/output is reconstructed from root observations, the Metrics `traces` view is replaced by the `observations` view with trace-level dimensions, and dataset runs become experiments queried by dataset ID.
